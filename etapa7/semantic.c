...
void check_and_set_declarations(AST* node) {
    ...
    switch (node->type) {
        ...
        case AST_DECLARATION_GLOBAL_INT:
            if (node->symbol)  
                if(node->symbol->type != TK_IDENTIFIER && node->symbol->type != SYMBOL_FUNCTION_ARGS) {
        ...
        case AST_DECLARATION_FUNCTION_ARGS_INT:
            if (node->symbol)
                if(node->symbol->type != TK_IDENTIFIER && node->symbol->type != SYMBOL_VARIABLE) {
...
void set_datatype(AST* node){
    switch (node->type) {
        ...
        case AST_DECLARATION_FUNCTION_ARGS_FLOAT:
        case AST_DECLARATION_GLOBAL_FLOAT:          node->symbol->datatype = DATATYPE_FLOAT         ; node->symbol->datavalue = atoi(node->son[0]?node->son[0]->symbol->text:"0"); node->symbol->datastring = node->son[1]?node->son[1]->symbol->text:"";  break;

        case AST_DECLARATION_FUNCTION_ARGS_INT:
        case AST_DECLARATION_GLOBAL_INT:            node->symbol->datatype = DATATYPE_INT           ; node->symbol->datavalue = atoi(node->son[0]?node->son[0]->symbol->text:"0"); break;
